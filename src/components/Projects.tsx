    ...
    >
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col items-center text-center gap-3 mb-16">
          ...
          <p className="max-w-2xl text-slate-600 dark:text-slate-300 text-sm sm:text-base leading-relaxed font-light">
        ...
        <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project) => (
            <div 
              key={project.id} 
              className="group relative rounded-[2rem] bg-white/90 dark:bg-gradient-to-b dark:from-void-2/90 dark:to-void-3/90 border border-slate-200 dark:border-white/10 p-5 backdrop-blur-xl transition-all duration-500 hover:border-cyan/40 hover:shadow-2xl hover:shadow-cyan/10 hover:-translate-y-1.5 flex flex-col justify-between"
            >
              <div>
                {/* Dribbble Style Hero Image Container */}
                <div className="w-full h-52 rounded-2xl overflow-hidden mb-5 relative bg-slate-900 border border-white/5">
                  {/* Category Pill */}
                ...
                <h3 className="text-xl font-bold font-display text-slate-900 dark:text-white mb-2 group-hover:text-cyan transition-colors">
                  {project.title}
                </h3>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5 mb-6">
                  {project.tags.map((tag, idx) => (
                    <span 
                      key={idx} 
                      className="px-2.5 py-1 bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-full text-[10px] font-mono text-slate-700 dark:text-slate-300 group-hover:border-cyan/30 transition-colors"
                    >
                      {tag}
                    </span>
                  ))}
                ...
                <div className="flex items-center gap-3 border-t border-slate-200 dark:border-white/10 pt-4">
                  <a 
                    ...
                    target="_blank" 
                    rel="noreferrer" 
                    className="flex-1 py-2.5 px-4 bg-gradient-to-r from-cyan to-indigo-500 dark:from-accent-start dark:to-accent-end hover:from-indigo-600 hover:to-cyan text-white dark:text-void font-bold text-xs rounded-full transition-all text-center flex items-center justify-center gap-1.5 shadow-md shadow-cyan/10 group/btn"
                  >
                    <span>Live App</span>
                  ...
                  {project.github && (
                    <a 
                      ...
                      target="_blank" 
                      rel="noreferrer" 
                      className="py-2.5 px-3.5 bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 border border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white font-medium text-xs rounded-full transition-colors flex items-center gap-1.5"
                    >
